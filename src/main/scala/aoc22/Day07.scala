...
object Day07A extends App {
  ...
  val (files, _) = input.foldLeft(Map.empty[String, Long], "") { case ((filesAcc, path), command) =>
    command match {
      case s"$$ cd .." =>
  ...
  val trie = files.foldLeft(FileNode("", None, Map.empty)) { case (t, entry) =>
    t.put(entry._1.split("/"), entry._2)
  }
  val smallTotal = FileTrie(trie.get(Seq("")))
    .collectDirectories()
    .toSeq
    .map(_.size)
    .filter(_ < 100000L)
...
object Day07B extends App {
  ...
  val candidate = FileTrie(trie.get(Seq("")))
    .collectDirectories()
    .toSeq
    .map(_.size)
    .filter(_ >= reqSpace)
