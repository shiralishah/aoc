...
object Day10A extends App {
  ...
  val (_, _, strengths) = input.foldLeft(1L, 1, Seq.empty[Long]) {
    case((x,cycle, strengthList), inst) =>
...
object Day10B extends App {
  ...
  val (_, _, finalPic) = input.foldLeft(1L, 1, Array.fill(6)(Array.fill(40)('.'))) {
    case((x,cycle, picture), inst) =>
      val i = (cycle-1) % width
      val j = cycle / width
