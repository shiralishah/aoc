      ...
      .foldLeft(Seq.empty[Int], Seq.empty[Packet], Option.empty[NumberPacket]) {
        case ((stack, packets, lastNum), (char, ind)) =>
          char match {
            ...
            case ']' =>
              val newPackets = lastNum.map(n =>
                (packets.head :++ n) +: packets.drop(1)
              ).getOrElse(packets)
...
object Day13A extends App {
  val input =
    scala.io.Source.fromResource("aoc22/run.txt")
      .mkString.split("\n\n").map(_.split("\n"))

  val (correct, _) = input.foldLeft(Seq.empty[Int], 1) {
    case ((indices, num), pair) =>
